...
@section('content')
    <div class="flex flex-col items-center justify-center px-5">
        ...
        <form action="{{route('evaluation.store')}}" method="POST">
            ...
            @if ($errors->has('id_user_coevaluator'))
                <div class="alert alert-danger mb-1 text-center font-bold">{{ $errors->first('id_user_coevaluator') }}</div>
            @endif
            ...
            <div class="rating mt-10">
                ...
                @endforeach
                <button type="submit" class="bg-gray-900 text-white text-sm font-light  py-2 px-4 rounded-lg mx-auto block">Enviar coevaluación</button>
            </div>
        </form>
