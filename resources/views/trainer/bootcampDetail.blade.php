    ...
    <div class="flex flex-col items-center justify-center">
        ...
        <div class="flex-col gap-3 mt-10">
            <a type="button" href="{{$promotions->zoom_url}}" target="_blank"
                ...
                Zoom
                <svg class="w-9 h-7" aria-hidden="true" focusable="false"
            ...
            <a type="button" href="{{$promotions->slack_url}}" target="_blank"
                ...
                Slack
                <svg class="w-7 h-7" aria-hidden="true" focusable="false"
        ...
        <table class="table-auto border-2 border-collapse border-orange-600 rounded-lg">
            @foreach ($coders->chunk(2) as $chunk)
                <tr>
                    @foreach ($chunk as $coder)
                    <td class="border-2 px-4 py-2 border-orange-600 rounded-lg">
                        <div class="flex justify-left text-gray-900 mt-2">
                            <p class="mr-1">{{$coder->name}}</p>
                        </div> 
                    </td>
                    @endforeach
                </tr>
        ...
        <div class="mt-5">
            <a type="button" value="Volver" href="{{route('trainer.promotions')}}" class="bg-black text-white text-sm font-light py-2 px-4 mt-4 rounded-lg mx-auto block w-24 text-center">Volver</a>
        </div>
    </div>
@endsection